...
[define [align->sxml lX lY nX nY diff-gap diff-mod]
  [if [null? lX]
    [cons [reverse [map mk-string-helper nX]]
          [reverse [map mk-string-helper nY]]]
    [if [equal? [car lX][car lY]]
      [align->sxml [cdr lX][cdr lY]
                   [cons-helper 'char [car lX] nX diff-gap diff-mod]
                   [cons-helper 'char [car lY] nY diff-gap diff-mod]
                   diff-gap diff-mod] 
      [if [equal? [car lX] 'gap]
        [align->sxml [cdr lX][cdr lY]
                     nX
                     [cons-helper 'diff-mod [car lY] nY diff-gap diff-mod]
                     diff-gap diff-mod] 
        [if [equal? [car lY] 'gap]
          [align->sxml [cdr lX] [cdr lY]
                       [cons-helper 'diff-mod [car lX] nX diff-gap diff-mod]
                       nY
                       diff-gap diff-mod] 
          [align->sxml [cdr lX][cdr lY]
                       [cons-helper 'diff-gap [car lX] nX diff-gap diff-mod]
                       [cons-helper 'diff-gap [car lY] nY diff-gap diff-mod]
                       diff-gap diff-mod]]]]]] 

[define [mk-string-helper x]
  [if [char? [car x]]
    [list->string [reverse x]]
    [[car x] [list->string [reverse [cdr x]]]]]]

[define [cons-helper s c cl diff-gap diff-mod]
  [if [equal? s 'char]
    [if [null? cl]
      [list [list c]]
      [if [char? [caar cl]]
...
[define [string-diff x y diff-gap diff-mod] 
  [let [[r [align x y -1 sim]]]
    [let [[nr [align->sxml [car r][cdr r] [list] [list] diff-gap diff-mod]]]
      [list [car nr] [cdr nr]]]]]

[define [diff-gap-def x]
[list 'span [list '@ [list 'class "diff-gap"]] x]]
[define [diff-mod-def x]
[list 'span [list '@ [list 'class "diff-mod"]] x]]

[define [test-NW]
  [let [[r [align "skgosyenjhdfognrarhdy" "skfgosgenjhdfoffgnrhy" -1 sim]]]
    [displayln r]
    [displayln [list->string [map [lambda [x] [if [equal? x 'gap] #\- x]] [car r]]]]
    [displayln [list->string [map [lambda [x] [if [equal? x 'gap] #\- x]] [cdr r]]]]
    [let [[nr [align->sxml [car r][cdr r] [list] [list] diff-gap-def diff-mod-def]]]
